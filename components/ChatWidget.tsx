...
export function ChatWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[99999] flex flex-col items-end pointer-events-none">
      {/* Chat Window */}
      <div 
        className={`mb-4 w-[350px] overflow-hidden rounded-2xl bg-white shadow-[0_20px_50px_rgba(0,0,0,0.2)] border border-slate-100 flex-col h-[500px] pointer-events-auto transition-all duration-300 origin-bottom-right ${isOpen ? 'flex opacity-100 scale-100' : 'hidden opacity-0 scale-90 pointer-events-none'}`}
      >
        {/* Header */}
        <div className="bg-[#1F2C30] p-4 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-full bg-[#41bf63] flex items-center justify-center text-black">
      ...
      <button
        ...
        className="pointer-events-auto h-16 w-16 rounded-full bg-[#1F2C30] shadow-2xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 group relative overflow-hidden"
      >
        <div className="absolute inset-0 bg-[#41bf63] translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
